...
# NP = emoticons
def pos_feature_builder(text,target_pos=('JJ','NN','VB','!','NP','RB','CD')):
	if not text:
		return patvec.count([])
	try:

		parsed_text = paten.parsetree(text,lemmata=True)[0]
		selected = [word.lemma for word in parsed_text if word.tag.startswith(target_pos)]
...
def postag_feature_builder(text,target_pos=('JJ','NN','VB','NP','RB','CD')):
	if not text:
		return {}
	try:

		result = patvec.count((word for word,tag in paten.tag(text, tokenize=True, encoding='utf-8') if tag in target_pos))
	except IndexError as e:
		print text,e
		result = {}
	return result
