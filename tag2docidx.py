...
class Tag2Document:
	...
	def __str__(self):
		return "tag = {0}, count = {1}, save = {2}, max_docs = {3}".format(self.tag, self.count, self.save_dir, self.max_documents)
	...
	def create_tag_media(self):
		...
		self.tag_media = loadpickle.get_all_tagpickles(self.tag, self.count,self.save_dir)
	...
	def create_tag_docs(self):
		...
		if self.mode =='twitter':
			tag_docs = tweet2document.create_tweet_document_index(self.tag_media,self.label,self.max_documents) 
		elif self.mode =='instagram':
			tag_docs = instagram2document.create_instagram_document_index(self.tag_media,self.label,self.max_documents)
